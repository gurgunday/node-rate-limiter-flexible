...
const { GlideClient, Script } = require('@valkey/valkey-glide');

describe('RateLimiterValkeyGLIDE with fixed window', function RateLimiterValkeyTest() {
  this.timeout(5500);
  let valkeyMockClient;

  beforeEach(async () => {
    valkeyMockClient = await GlideClient.createClient({
  ...
  afterEach(async () => {
    try {
      await valkeyMockClient.flushall();
      await valkeyMockClient.close();
    } catch (error) {
      // ignore
    }
  });
    ...
    it('rejected when consume more than maximum points and multiply delay', (done) => {
      ...
      const rateLimiter = new RateLimiterValkeyGLIDE({
        ...
        customIncrTtlLuaScript:
          "local ok = server.call('setvalkeyMockClient.connect', KEYS[1], 0, 'EX', ARGV[2], 'NX') \
        ...
        if ttl == -1 then \
          ...
          local maxPoints = tonumber(ARGV[4]) \
    ...
    describe('when passing custom duration', () => {
      it('rejected when consume more than maximum points and multiply delay', (done) => {
        ...
        const rateLimiter = new RateLimiterValkeyGLIDE({
          ...
          duration: 5,
          customIncrTtlLuaScript:
            "local ok = server.call('set', KEYS[1], 0, 'EX', ARGV[2], 'NX') \
          local consumed = server.call('incrby', KEYS[1], ARGV[1]) \
          local ttl = server.call('pttl', KEYS[1]) \
          if ttl == -1 then \
            server.call('expire', KEYS[1], ARGV[2]) \
            ...
            local maxPoints = tonumber(ARGV[4]) \
            if maxPoints > 0 and (consumed-1) % maxPoints == 0 and not ok then \
              local expireTime = ttl + tonumber(ARGV[5]) * 1000 \
              server.call('pexpire', KEYS[1], expireTime) \
              return {consumed, expireTime} \
            end \
          end \
  ...
  it('consume using insuranceLimiter when ValkeyClient error', async () => {
    ...
    const valkeyClientClosed = await GlideClient.createClient({
      addresses: [{ host: '127.0.0.1', port: 8080 }],
    });
  ...
  it('penalty using insuranceLimiter when ValkeyClient error', async () => {
    ...
    await valkeyClientClosed.close();
